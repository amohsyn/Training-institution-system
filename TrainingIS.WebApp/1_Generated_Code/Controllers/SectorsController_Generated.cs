...
{  
    ...
    public class BaseSectorsController : TrainingIS_BaseController
    {
		...
		public virtual ActionResult Index(FilterRequestParams filterRequestParams)
        {
            ...
            List<string> SearchCreteria = this.SectorBLO.GetSearchCreteria();
        ...
        {
			if(Default_Sector_Edit_Model.Id == 0)
            {
                string msg = string.Format(msgManager.You_try_to_edit_that_does_not_exist, msgHelper.UndefindedArticle(), msg_Sector.SingularName.ToLower());
                Alert(msg, NotificationType.error);
                return RedirectToAction("Index");
        ...
        /// <summary>
        /// [Generalize] Export the filterted Sectors
        /// </summary>
        /// <returns></returns>
        public virtual FileResult Export()
        {
            var dataTable = this.SectorBLO.Export(this.GetType().Name);
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    string FileName = SectorBLO.Get_Export_File_Name();
        ...
        public virtual FileResult Import_File_Example()
        {
            using (XLWorkbook wb = new XLWorkbook())
            {
                DataTable dataTable = SectorBLO.Import_File_Example();
                wb.Worksheets.Add(dataTable);
                using (MemoryStream stream = new MemoryStream())
                {
                    wb.SaveAs(stream);
                    string FileName = this.SectorBLO.Get_Import_File_Name();
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", FileName);
                }
            }
        }
