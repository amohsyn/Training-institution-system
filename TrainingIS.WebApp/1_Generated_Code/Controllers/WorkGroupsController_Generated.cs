...
{  
    ...
    public class BaseWorkGroupsController : TrainingIS_BaseController
    {
		...
		public virtual ActionResult Index(FilterRequestParams filterRequestParams)
        {
            ...
            List<string> SearchCreteria = this.WorkGroupBLO.GetSearchCreteria();
        ...
        /// <summary>
        /// [Generalize] Export the filterted WorkGroups
        /// </summary>
        /// <returns></returns>
        public virtual FileResult Export()
        {
            var dataTable = this.WorkGroupBLO.Export(this.GetType().Name);
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    string FileName = WorkGroupBLO.Get_Export_File_Name();
        ...
        public virtual FileResult Import_File_Example()
        {
            using (XLWorkbook wb = new XLWorkbook())
            {
                DataTable dataTable = WorkGroupBLO.Import_File_Example();
                wb.Worksheets.Add(dataTable);
                using (MemoryStream stream = new MemoryStream())
                {
                    wb.SaveAs(stream);
                    string FileName = this.WorkGroupBLO.Get_Import_File_Name();
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", FileName);
                }
            }
        }
