...
{  
    ...
    public class BaseAttendanceStatesController : TrainingIS_BaseController
    {
        protected AttendanceStateBLO AttendanceStateBLO = null;

		public BaseAttendanceStatesController()
        {
            this.msgHelper = new MessagesService(typeof(AttendanceState));
			this.AttendanceStateBLO = new AttendanceStateBLO(this._UnitOfWork, this.GAppContext) ;
        }
        ...
        protected virtual void InitFilter(Index_GAppPage index_page, string FilterBy,string SearchBy)
        {
			...
			string All_Trainee_msg = string.Format("tous les {0}",msg_AttendanceState.PluralName.ToLower());
		...
		public virtual ActionResult Index(FilterRequestParams filterRequestParams)
        {
            ...
            List<string> SearchCreteria = this.AttendanceStateBLO.GetSearchCreteria();
		...
		public virtual ActionResult Create(Default_AttendanceState_Create_Model Default_AttendanceState_Create_Model)
        {
            ...
            if (ModelState.IsValid)
            {
				...
				try
                {
                    AttendanceStateBLO.Save(AttendanceState);
					Alert(string.Format(msgManager.The_Entity_was_well_created, msgHelper.DefinitArticle().FirstLetterToUpperCase(), msg_AttendanceState.SingularName.ToLower(), AttendanceState), NotificationType.success);
		...
		public virtual ActionResult Edit(long? id)
        {
            ...
            AttendanceState AttendanceState = AttendanceStateBLO.FindBaseEntityByID((long)id);
		...
		public virtual ActionResult Edit(Default_AttendanceState_Edit_Model Default_AttendanceState_Edit_Model)	
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    AttendanceStateBLO.Save(AttendanceState);
					Alert(string.Format(msgManager.The_entity_has_been_changed,msgHelper.DefinitArticle().FirstLetterToUpperCase(), msg_AttendanceState.SingularName.ToLower(), AttendanceState), NotificationType.success);
					return RedirectToAction("Index");
                }
                catch (GAppException ex)
                {
		...
		public virtual ActionResult Details(long? id)
        {
            ...
            if (AttendanceState == null)
            {
                string msg = string.Format(msgManager.You_try_to_show_that_does_not_exist, msgHelper.UndefindedArticle(), msg_AttendanceState.SingularName.ToLower());
                Alert(msg, NotificationType.error);
                return RedirectToAction("Index");
            }
		 ...
		 public virtual ActionResult Delete(long? id)
        {
            ...
            if (AttendanceState == null)
            {
			    string msg = string.Format(msgManager.You_try_to_delete_that_does_not_exist, msgHelper.UndefindedArticle(), msg_AttendanceState.SingularName.ToLower());
                Alert(msg, NotificationType.error);
                return RedirectToAction("Index");
            }
        ...
        public virtual ActionResult DeleteConfirmed(long id)
        {
            ...
            {
                string msg = string.Format(msgManager.You_can_not_delete_this_entity_because_it_is_already_related_to_another_object, AttendanceState.ToString());
                Alert(msg, NotificationType.error);
                return RedirectToAction("Index");
            }
            catch (GAppException ex)
			...
			Alert(string.Format(msgManager.The_entity_has_been_removed,msgHelper.DefinitArticle().FirstLetterToUpperCase(), msg_AttendanceState.SingularName.ToLower(), AttendanceState), NotificationType.success);
            return RedirectToAction("Index");
        }
        ...
        { 
            ...
            try
            {
                ImportReport importReport = AttendanceStateBLO.Import(firstTable, FileName);
                ...
                using (XLWorkbook wb = new XLWorkbook())
                {
					...
					Session["repport_name"] = msg_AttendanceState.PluralName;
                    wb.SaveAs(path_repport);

                    // Add DownLoad Link to Repport
                    string a_download = "<a href=\"/AttendanceStates/LastRepportFile\">Télécharger le rapport d'importation</a>";
        ...
        /// <summary>
        /// [Generalize] Export the filterted AttendanceStates
        /// </summary>
        /// <returns></returns>
        public virtual FileResult Export()
        {
            var dataTable = this.AttendanceStateBLO.Export(this.GetType().Name);
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    string FileName = AttendanceStateBLO.Get_Export_File_Name();
        ...
        public virtual FileResult Import_File_Example()
        {
            using (XLWorkbook wb = new XLWorkbook())
            {
                DataTable dataTable = AttendanceStateBLO.Import_File_Example();
                wb.Worksheets.Add(dataTable);
                using (MemoryStream stream = new MemoryStream())
                {
                    wb.SaveAs(stream);
                    string FileName = this.AttendanceStateBLO.Get_Import_File_Name();
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", FileName);
                }
            }
        }
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                AttendanceStateBLO.Dispose();
            }
            base.Dispose(disposing);
        }
    }

	public partial class AttendanceStatesController : BaseAttendanceStatesController{};
}
