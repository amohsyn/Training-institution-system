...
{  
    ...
    public class BaseSeanceTrainingsController : TrainingIS_BaseController
    {
		...
		public virtual ActionResult Index(FilterRequestParams filterRequestParams)
        {
            ...
            List<Index_SeanceTraining_Model> _ListIndex_SeanceTraining_Model = null;
            try
            {
                filterRequestParams = this.Save_OR_Load_filterRequestParams_State(filterRequestParams);
               _ListIndex_SeanceTraining_Model = new Index_SeanceTraining_ModelBLM(this._UnitOfWork, this.GAppContext)
        ...
        /// <summary>
        /// [Generalize] Export the filterted SeanceTrainings
        /// </summary>
        /// <returns></returns>
        public virtual FileResult Export()
        {
            var dataTable = this.SeanceTrainingBLO.Export(this.GetType().Name);
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    string FileName = SeanceTrainingBLO.Get_Export_File_Name();
        ...
        public virtual FileResult Import_File_Example()
        {
            using (XLWorkbook wb = new XLWorkbook())
            {
                DataTable dataTable = SeanceTrainingBLO.Import_File_Example();
                wb.Worksheets.Add(dataTable);
                using (MemoryStream stream = new MemoryStream())
                {
                    wb.SaveAs(stream);
                    string FileName = this.SeanceTrainingBLO.Get_Import_File_Name();
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", FileName);
                }
            }
        }
