...
{  
    ...
    public class BaseAbsencesController : TrainingIS_BaseController
    {
		...
		public virtual ActionResult Index(FilterRequestParams filterRequestParams)
        {
            ...
            List<string> SearchCreteria = this.AbsenceBLO.GetSearchCreteria();
        ...
        /// <summary>
        /// [Generalize] Export the filterted Absences
        /// </summary>
        /// <returns></returns>
        public virtual FileResult Export()
        {
            var dataTable = this.AbsenceBLO.Export(this.GetType().Name);
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    string FileName = AbsenceBLO.Get_Export_File_Name();
        ...
        public virtual FileResult Import_File_Example()
        {
            using (XLWorkbook wb = new XLWorkbook())
            {
                DataTable dataTable = AbsenceBLO.Import_File_Example();
                wb.Worksheets.Add(dataTable);
                using (MemoryStream stream = new MemoryStream())
                {
                    wb.SaveAs(stream);
                    string FileName = this.AbsenceBLO.Get_Import_File_Name();
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", FileName);
                }
            }
        }
