...
{ 
	public partial class BaseEntityPropertyShortcutBLO : BaseBLO<EntityPropertyShortcut>{
		...
		public BaseEntityPropertyShortcutBLO(UnitOfWork UnitOfWork) : base(UnitOfWork.EntityPropertyShortcutDAO)
        {
		...
		public override List<EntityPropertyShortcut> FindAll()
        {
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_EntityPropertyShortcut.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(EntityPropertyShortcut));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(EntityPropertyShortcut));
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(EntityPropertyShortcut), navigationPropertiesNames, foreignKeys);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new EntityPropertyShortcutDAO(unitOfWorkImport.context);
        }
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(EntityPropertyShortcut entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(EntityPropertyShortcut entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                EntityPropertyShortcut EntityPropertyShortcut_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(EntityPropertyShortcut_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(EntityPropertyShortcut));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<EntityPropertyShortcut, bool>>(body, param);

                var exsitant_entity = context.EntityPropertyShortcuts.Where(lambda_expression).FirstOrDefault();
