...
{
	public partial class BaseAppControllerActionBLO : BaseBLO<AppControllerAction>{
	    
		protected UnitOfWork _UnitOfWork = null;

		public BaseAppControllerActionBLO(UnitOfWork UnitOfWork) : base()
        {
		    this._UnitOfWork = UnitOfWork;
            this.entityDAO = this._UnitOfWork.AppControllerActionDAO;
        }
		 
		private BaseAppControllerActionBLO() : base() {}
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_AppControllerAction.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(AppControllerAction));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(AppControllerAction));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(AppControllerAction).GetProperties();
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(AppControllerAction), navigationPropertiesNames, foreignKeys);
            ...
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                Operation operation;
                AppControllerAction entity = this.Load_Or_CreateEntity(importService, entity_reference);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new AppControllerActionDAO(unitOfWorkImport.context);
        }
        private AppControllerAction Load_Or_CreateEntity(ImportService importService, string entity_reference)
        {
            Operation operation;
            AppControllerAction entity = this.FindBaseEntityByReference(entity_reference);
            if (entity == null) // Add new if the entity not exist
            {
                entity = new AppControllerAction();
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(AppControllerAction entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(AppControllerAction entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                AppControllerAction AppControllerAction_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(AppControllerAction_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(AppControllerAction));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<AppControllerAction, bool>>(body, param);

                var exsitant_entity = context.AppControllerActions.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class AppControllerActionBLO : BaseAppControllerActionBLO{
		public AppControllerActionBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
