...
namespace  TrainingIS.BLL
{
	public partial class BaseFormerBLO : BaseBLO<Former>{
		...
		public BaseFormerBLO(UnitOfWork UnitOfWork) : base()
        {
		    this._UnitOfWork = UnitOfWork;
            this.entityDAO = this._UnitOfWork.FormerDAO;
        }
		 
		private BaseFormerBLO() : base() {}
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_Former.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(Former));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(Former));
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(Former entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(Former entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                Former Former_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(Former_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(Former));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<Former, bool>>(body, param);

                var exsitant_entity = context.Formers.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class FormerBLO : BaseFormerBLO{
		public FormerBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
