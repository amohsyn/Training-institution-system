...
namespace  TrainingIS.BLL
{
	public partial class BaseRoleAppBLO : BaseBLO<RoleApp>{
		...
		public BaseRoleAppBLO(UnitOfWork UnitOfWork) : base()
        {
		    this._UnitOfWork = UnitOfWork;
            this.entityDAO = this._UnitOfWork.RoleAppDAO;
        }
		 
		private BaseRoleAppBLO() : base() {}
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_RoleApp.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(RoleApp));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(RoleApp));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(RoleApp).GetProperties();
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(RoleApp), navigationPropertiesNames, foreignKeys);
            ...
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                Operation operation;
                RoleApp entity = this.Load_Or_CreateEntity(importService, entity_reference);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new RoleAppDAO(unitOfWorkImport.context);
        }
        private RoleApp Load_Or_CreateEntity(ImportService importService, string entity_reference)
        {
            Operation operation;
            RoleApp entity = this.FindBaseEntityByReference(entity_reference);
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(RoleApp entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(RoleApp entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                RoleApp RoleApp_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(RoleApp_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(RoleApp));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<RoleApp, bool>>(body, param);

                var exsitant_entity = context.RoleApps.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class RoleAppBLO : BaseRoleAppBLO{
		public RoleAppBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
