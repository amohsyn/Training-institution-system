...
{ 
	public partial class BaseTrainingBLO : BaseBLO<Training>{
	    
		protected UnitOfWork _UnitOfWork = null;

		public BaseTrainingBLO(UnitOfWork UnitOfWork) : base(UnitOfWork.TrainingDAO)
        {
		...
		public override List<Training> FindAll()
        {
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_Training.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(Training));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(Training));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(Training).GetProperties();
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(Training), navigationPropertiesNames, foreignKeys);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new TrainingDAO(unitOfWorkImport.context);
        }
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(Training entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(Training entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                Training Training_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(Training_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(Training));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<Training, bool>>(body, param);

                var exsitant_entity = context.Trainings.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class TrainingBLO : BaseTrainingBLO{
		public TrainingBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
