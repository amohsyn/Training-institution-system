...
{ 
	public partial class BaseSeancePlanningBLO : BaseBLO<SeancePlanning>{
		...
		public BaseSeancePlanningBLO(UnitOfWork UnitOfWork) : base(UnitOfWork.SeancePlanningDAO)
        {
		...
		public override List<SeancePlanning> FindAll()
        {
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_SeancePlanning.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(SeancePlanning));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(SeancePlanning));
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(SeancePlanning), navigationPropertiesNames, foreignKeys);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new SeancePlanningDAO(unitOfWorkImport.context);
        }
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(SeancePlanning entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(SeancePlanning entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                SeancePlanning SeancePlanning_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(SeancePlanning_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(SeancePlanning));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<SeancePlanning, bool>>(body, param);

                var exsitant_entity = context.SeancePlannings.Where(lambda_expression).FirstOrDefault();
