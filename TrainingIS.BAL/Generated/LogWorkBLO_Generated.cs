...
namespace  TrainingIS.BLL
{
	public partial class BaseLogWorkBLO : BaseBLO<LogWork>{
		...
		public BaseLogWorkBLO(UnitOfWork UnitOfWork) : base()
        {
		    this._UnitOfWork = UnitOfWork;
            this.entityDAO = this._UnitOfWork.LogWorkDAO;
        }
		 
		private BaseLogWorkBLO() : base() {}
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_LogWork.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(LogWork));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(LogWork));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(LogWork).GetProperties();
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(LogWork), navigationPropertiesNames, foreignKeys);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new LogWorkDAO(unitOfWorkImport.context);
        }
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(LogWork entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(LogWork entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                LogWork LogWork_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(LogWork_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(LogWork));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<LogWork, bool>>(body, param);

                var exsitant_entity = context.LogWorks.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class LogWorkBLO : BaseLogWorkBLO{
		public LogWorkBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
