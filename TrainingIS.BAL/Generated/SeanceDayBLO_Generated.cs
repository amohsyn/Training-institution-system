...
{ 
	public partial class BaseSeanceDayBLO : BaseBLO<SeanceDay>{
		...
		public BaseSeanceDayBLO(UnitOfWork UnitOfWork) : base(UnitOfWork.SeanceDayDAO)
        {
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_SeanceDay.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(SeanceDay));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(SeanceDay));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(SeanceDay).GetProperties();
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(SeanceDay), navigationPropertiesNames, foreignKeys);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new SeanceDayDAO(unitOfWorkImport.context);
        }
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(SeanceDay entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(SeanceDay entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                SeanceDay SeanceDay_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(SeanceDay_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(SeanceDay));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<SeanceDay, bool>>(body, param);

                var exsitant_entity = context.SeanceDays.Where(lambda_expression).FirstOrDefault();
