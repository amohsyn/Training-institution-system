...
{
	public partial class BaseAppControllerBLO : BaseBLO<AppController>{
	    
		protected UnitOfWork _UnitOfWork = null;

		public BaseAppControllerBLO(UnitOfWork UnitOfWork) : base()
        {
		    this._UnitOfWork = UnitOfWork;
            this.entityDAO = this._UnitOfWork.AppControllerDAO;
        }
		 
		private BaseAppControllerBLO() : base() {}
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_AppController.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(AppController));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(AppController));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(AppController).GetProperties();
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(AppController), navigationPropertiesNames, foreignKeys);
            ...
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                Operation operation;
                AppController entity = this.Load_Or_CreateEntity(importService, entity_reference);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new AppControllerDAO(unitOfWorkImport.context);
        }
        private AppController Load_Or_CreateEntity(ImportService importService, string entity_reference)
        {
            Operation operation;
            AppController entity = this.FindBaseEntityByReference(entity_reference);
            if (entity == null) // Add new if the entity not exist
            {
                entity = new AppController();
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(AppController entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(AppController entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                AppController AppController_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(AppController_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(AppController));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<AppController, bool>>(body, param);

                var exsitant_entity = context.AppControllers.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class AppControllerBLO : BaseAppControllerBLO{
		public AppControllerBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
