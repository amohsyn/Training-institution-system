...
{ 
	public partial class BaseScheduleBLO : BaseBLO<Schedule>{
		...
		public BaseScheduleBLO(UnitOfWork UnitOfWork) : base(UnitOfWork.ScheduleDAO)
        {
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_Schedule.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(Schedule));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(Schedule));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(Schedule).GetProperties();
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(Schedule), navigationPropertiesNames, foreignKeys);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new ScheduleDAO(unitOfWorkImport.context);
        }
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(Schedule entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(Schedule entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                Schedule Schedule_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(Schedule_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(Schedule));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<Schedule, bool>>(body, param);

                var exsitant_entity = context.Schedules.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class ScheduleBLO : BaseScheduleBLO{
		public ScheduleBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
