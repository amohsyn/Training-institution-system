...
{
	public partial class BaseClassroomCategoryBLO : BaseBLO<ClassroomCategory>{
	    
		protected UnitOfWork _UnitOfWork = null;

		public BaseClassroomCategoryBLO(UnitOfWork UnitOfWork) : base()
        {
		    this._UnitOfWork = UnitOfWork;
            this.entityDAO = this._UnitOfWork.ClassroomCategoryDAO;
        }
		 
		private BaseClassroomCategoryBLO() : base() {}
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_ClassroomCategory.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(ClassroomCategory));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(ClassroomCategory));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(ClassroomCategory).GetProperties();
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(ClassroomCategory), navigationPropertiesNames, foreignKeys);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new ClassroomCategoryDAO(unitOfWorkImport.context);
        }
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(ClassroomCategory entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(ClassroomCategory entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                ClassroomCategory ClassroomCategory_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(ClassroomCategory_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(ClassroomCategory));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<ClassroomCategory, bool>>(body, param);

                var exsitant_entity = context.ClassroomCategories.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class ClassroomCategoryBLO : BaseClassroomCategoryBLO{
		public ClassroomCategoryBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
