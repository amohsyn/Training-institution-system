...
{
	public partial class BaseAppRoleBLO : BaseBLO<AppRole>{
	    
		protected UnitOfWork _UnitOfWork = null;

		public BaseAppRoleBLO(UnitOfWork UnitOfWork) : base()
        {
		    this._UnitOfWork = UnitOfWork;
            this.entityDAO = this._UnitOfWork.AppRoleDAO;
        }
		 
		private BaseAppRoleBLO() : base() {}
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_AppRole.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(AppRole));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(AppRole));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(AppRole).GetProperties();
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(AppRole), navigationPropertiesNames, foreignKeys);
            ...
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                Operation operation;
                AppRole entity = this.Load_Or_CreateEntity(importService, entity_reference);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new AppRoleDAO(unitOfWorkImport.context);
        }
        private AppRole Load_Or_CreateEntity(ImportService importService, string entity_reference)
        {
            Operation operation;
            AppRole entity = this.FindBaseEntityByReference(entity_reference);
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(AppRole entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(AppRole entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                AppRole AppRole_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(AppRole_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(AppRole));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<AppRole, bool>>(body, param);

                var exsitant_entity = context.AppRoles.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class AppRoleBLO : BaseAppRoleBLO{
		public AppRoleBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
