...
{
	public partial class BaseSeanceTrainingBLO : BaseBLO<SeanceTraining>{
	    
		protected UnitOfWork _UnitOfWork = null;

		public BaseSeanceTrainingBLO(UnitOfWork UnitOfWork) : base()
        {
		    this._UnitOfWork = UnitOfWork;
            this.entityDAO = this._UnitOfWork.SeanceTrainingDAO;
        }
		 
		private BaseSeanceTrainingBLO() : base() {}
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_SeanceTraining.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(SeanceTraining));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(SeanceTraining));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(SeanceTraining).GetProperties();
	...
	public virtual ImportReport Import(DataTable dataTable, string FileName = "")
    {
            ...
            ImportService importService = new ImportService(dataTable, typeof(SeanceTraining), navigationPropertiesNames, foreignKeys);
            ...
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                Operation operation;
                SeanceTraining entity = this.Load_Or_CreateEntity(importService, entity_reference);
        ...
        private void InitUnitOfWork()
        {
            ...
            this.entityDAO = new SeanceTrainingDAO(unitOfWorkImport.context);
        }
        private SeanceTraining Load_Or_CreateEntity(ImportService importService, string entity_reference)
        {
            Operation operation;
            SeanceTraining entity = this.FindBaseEntityByReference(entity_reference);
            if (entity == null) // Add new if the entity not exist
            {
                entity = new SeanceTraining();
        ...
        /// <returns></returns> 
        public IList<ValidationResult> Validate(SeanceTraining entity)
        {
        ...
        /// <returns></returns>
        public bool IsUnique(SeanceTraining entity, PropertyInfo propertyInfo)
        {
		    // Edit Case
            if (entity.Id != 0)
            {
                // Edit Case
                SeanceTraining SeanceTraining_db = this.FindBaseEntityByID(entity.Id);
                if (propertyInfo.GetValue(SeanceTraining_db).ToString() == propertyInfo.GetValue(entity).ToString())
                    return true;
            }
            ...
            if (propertyInfo.GetValue(entity) != null)
            {
                ...
                var param = Expression.Parameter(typeof(SeanceTraining));
                var body = Expression.Equal(Expression.Property(param, propertyInfo),
                    Expression.Constant(propertyInfo.GetValue(entity)));
                var lambda_expression = Expression.Lambda<Func<SeanceTraining, bool>>(body, param);

                var exsitant_entity = context.SeanceTrainings.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class SeanceTrainingBLO : BaseSeanceTrainingBLO{
		public SeanceTrainingBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
