...
{ 
	public partial class BaseYearStudyBLO : TrainingIS_BaseBLO<YearStudy>{
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_YearStudy_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_YearStudy_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_YearStudy_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(YearStudy item)
        {
		...
		public virtual IQueryable<YearStudy> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<YearStudy, bool> Condition = null)
        {
           ...
           IQueryable<YearStudy> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(YearStudy));
            DataTable entityDataTable = exportService.CreateDataTable(msg_YearStudy.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_YearStudy_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_YearStudy_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_YearStudy.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(YearStudy), this.GAppContext);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new YearStudyDAO(_UnitOfWork.context);
			}
			private YearStudy Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				...
				{
					entity = new YearStudyBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual YearStudy Load_if_not_attached_in_current_context(YearStudy item)
			{

				YearStudy entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.YearStudies.Local.Where(a => a.Id == item.Id);
	...
	public  partial class YearStudyBLO : BaseYearStudyBLO{
		public YearStudyBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
