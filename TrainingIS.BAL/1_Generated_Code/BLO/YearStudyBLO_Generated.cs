...
{ 
	public partial class BaseYearStudyBLO : BaseBLO<YearStudy>{
		...
		public override List<YearStudy> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(YearStudy entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(YearStudy entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					YearStudy YearStudy_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(YearStudy_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(YearStudy));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<YearStudy, bool>>(body, param);

					var exsitant_entity = context.YearStudies.Where(lambda_expression).FirstOrDefault();
