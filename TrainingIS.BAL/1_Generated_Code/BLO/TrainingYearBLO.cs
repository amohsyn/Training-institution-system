...
{ 
	public partial class BaseTrainingYearBLO : BaseBLO<TrainingYear>{
		...
		public BaseTrainingYearBLO(UnitOfWork<TrainingISModel> UnitOfWork) : base(new TrainingYearDAO(UnitOfWork.context))
        {
		...
		public override List<TrainingYear> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(TrainingYear), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(TrainingYear entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(TrainingYear entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					TrainingYear TrainingYear_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(TrainingYear_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(TrainingYear));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<TrainingYear, bool>>(body, param);

					var exsitant_entity = context.TrainingYears.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class TrainingYearBLO : BaseTrainingYearBLO{
		public TrainingYearBLO(UnitOfWork<TrainingISModel> UnitOfWork) : base(UnitOfWork) {}
