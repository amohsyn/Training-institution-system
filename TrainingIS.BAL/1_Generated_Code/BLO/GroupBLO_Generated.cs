...
{ 
	public partial class BaseGroupBLO : TrainingIS_BaseBLO<Group>{
		...
		public BaseGroupBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new GroupDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_Group.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(IndexGroupView).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public virtual IQueryable<Group> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Group, bool> Condition = null)
        {
           ...
           IQueryable<Group> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Group));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Group.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Group_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Group_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Group.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Group), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Group entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new GroupDAO(_UnitOfWork.context);
			}
			private Group Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Group entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new GroupBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Group Load_if_not_attached_in_current_context(Group item)
			{

				Group entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Groups.Local.Where(a => a.Id == item.Id);
	...
	public  partial class GroupBLO : BaseGroupBLO{
		public GroupBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
