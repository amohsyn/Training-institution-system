...
{ 
	public partial class BaseGroupBLO : BaseBLO<Group>{
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(Group entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(Group entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					Group Group_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(Group_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(Group));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<Group, bool>>(body, param);

					var exsitant_entity = context.Groups.Where(lambda_expression).FirstOrDefault();
