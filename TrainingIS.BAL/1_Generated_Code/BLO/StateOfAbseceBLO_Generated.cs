...
{ 
	public partial class BaseStateOfAbseceBLO : TrainingIS_BaseBLO<StateOfAbsece>{
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_StateOfAbsece_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public virtual IQueryable<StateOfAbsece> Find_as_Queryable(
			...
			Func<StateOfAbsece, bool> Condition = null)
        {
           ...
           IQueryable<StateOfAbsece> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(StateOfAbsece));
            DataTable entityDataTable = exportService.CreateDataTable(msg_StateOfAbsece.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_StateOfAbsece_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_StateOfAbsece_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_StateOfAbsece.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(StateOfAbsece), this.GAppContext);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new StateOfAbseceDAO(_UnitOfWork.context);
			}
			private StateOfAbsece Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				...
				{
					entity = new StateOfAbseceBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual StateOfAbsece Load_if_not_attached_in_current_context(StateOfAbsece item)
			{

				StateOfAbsece entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.StateOfAbseces.Local.Where(a => a.Id == item.Id);
