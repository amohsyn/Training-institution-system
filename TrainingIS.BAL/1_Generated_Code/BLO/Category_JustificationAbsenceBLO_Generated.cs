...
{ 
	public partial class BaseCategory_JustificationAbsenceBLO : TrainingIS_BaseBLO<Category_JustificationAbsence>{
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Category_JustificationAbsence_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(Category_JustificationAbsence item)
        {
		...
		public virtual IQueryable<Category_JustificationAbsence> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Category_JustificationAbsence, bool> Condition = null)
        {
           ...
           IQueryable<Category_JustificationAbsence> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Category_JustificationAbsence));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Category_JustificationAbsence.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Category_JustificationAbsence_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Category_JustificationAbsence_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Category_JustificationAbsence.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Category_JustificationAbsence), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Category_JustificationAbsence entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new Category_JustificationAbsenceDAO(_UnitOfWork.context);
			}
			private Category_JustificationAbsence Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Category_JustificationAbsence entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new Category_JustificationAbsenceBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Category_JustificationAbsence Load_if_not_attached_in_current_context(Category_JustificationAbsence item)
			{

				Category_JustificationAbsence entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Category_JustificationAbsences.Local.Where(a => a.Id == item.Id);
