...
{ 
	public partial class BaseWorkGroupBLO : TrainingIS_BaseBLO<WorkGroup>{
	    

		public BaseWorkGroupBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new WorkGroupDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_WorkGroup.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_WorkGroup_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(WorkGroup item)
        {
		...
		public virtual IQueryable<WorkGroup> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<WorkGroup, bool> Condition = null)
        {
           ...
           IQueryable<WorkGroup> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(WorkGroup));
            DataTable entityDataTable = exportService.CreateDataTable(msg_WorkGroup.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_WorkGroup_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_WorkGroup_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_WorkGroup.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(WorkGroup), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					WorkGroup entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new WorkGroupDAO(_UnitOfWork.context);
			}
			private WorkGroup Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				WorkGroup entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new WorkGroupBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual WorkGroup Load_if_not_attached_in_current_context(WorkGroup item)
			{

				WorkGroup entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.WorkGroups.Local.Where(a => a.Id == item.Id);
