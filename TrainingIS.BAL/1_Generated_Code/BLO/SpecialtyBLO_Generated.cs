...
{ 
	public partial class BaseSpecialtyBLO : TrainingIS_BaseBLO<Specialty>{
	    

		public BaseSpecialtyBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new SpecialtyDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_Specialty.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_Specialty_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Specialty_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Specialty_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(Specialty item)
        {
		...
		public virtual IQueryable<Specialty> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Specialty, bool> Condition = null)
        {
           ...
           IQueryable<Specialty> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Specialty));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Specialty.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Specialty_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Specialty_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Specialty.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Specialty), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Specialty entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new SpecialtyDAO(_UnitOfWork.context);
			}
			private Specialty Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Specialty entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new SpecialtyBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Specialty Load_if_not_attached_in_current_context(Specialty item)
			{

				Specialty entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Specialties.Local.Where(a => a.Id == item.Id);
	...
	public  partial class SpecialtyBLO : BaseSpecialtyBLO{
		public SpecialtyBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
