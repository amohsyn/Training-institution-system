...
{ 
	public partial class BaseSpecialtyBLO : BaseBLO<Specialty>{
		...
		public override List<Specialty> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Specialty), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(Specialty entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(Specialty entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					Specialty Specialty_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(Specialty_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(Specialty));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<Specialty, bool>>(body, param);

					var exsitant_entity = context.Specialties.Where(lambda_expression).FirstOrDefault();
