...
{ 
	public partial class BaseScheduleBLO : TrainingIS_BaseBLO<Schedule>{
		...
		public BaseScheduleBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new ScheduleDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_Schedule.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Index_Schedule_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(Schedule item)
        {
		...
		public virtual IQueryable<Schedule> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Schedule, bool> Condition = null)
        {
           ...
           IQueryable<Schedule> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Schedule));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Schedule.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Schedule_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Schedule_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Schedule.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Schedule), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Schedule entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new ScheduleDAO(_UnitOfWork.context);
			}
			private Schedule Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Schedule entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new ScheduleBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Schedule Load_if_not_attached_in_current_context(Schedule item)
			{

				Schedule entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Schedules.Local.Where(a => a.Id == item.Id);
	...
	public  partial class ScheduleBLO : BaseScheduleBLO{
		public ScheduleBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
