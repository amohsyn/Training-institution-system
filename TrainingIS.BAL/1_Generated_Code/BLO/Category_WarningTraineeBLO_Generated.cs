...
{ 
	public partial class BaseCategory_WarningTraineeBLO : TrainingIS_BaseBLO<Category_WarningTrainee>{
		...
		public BaseCategory_WarningTraineeBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new Category_WarningTraineeDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_Category_WarningTrainee.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_Category_WarningTrainee_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Category_WarningTrainee_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Category_WarningTrainee_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(Category_WarningTrainee item)
        {
		...
		public virtual IQueryable<Category_WarningTrainee> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Category_WarningTrainee, bool> Condition = null)
        {
           ...
           IQueryable<Category_WarningTrainee> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Category_WarningTrainee));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Category_WarningTrainee.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Category_WarningTrainee_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Category_WarningTrainee_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Category_WarningTrainee.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Category_WarningTrainee), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Category_WarningTrainee entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new Category_WarningTraineeDAO(_UnitOfWork.context);
			}
			private Category_WarningTrainee Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Category_WarningTrainee entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new Category_WarningTraineeBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Category_WarningTrainee Load_if_not_attached_in_current_context(Category_WarningTrainee item)
			{

				Category_WarningTrainee entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Category_WarningTrainees.Local.Where(a => a.Id == item.Id);
	...
	public  partial class Category_WarningTraineeBLO : BaseCategory_WarningTraineeBLO{
		public Category_WarningTraineeBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
