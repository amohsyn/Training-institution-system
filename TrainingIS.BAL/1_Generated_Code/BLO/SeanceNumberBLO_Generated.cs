...
{ 
	public partial class BaseSeanceNumberBLO : BaseBLO<SeanceNumber>{
		...
		public override List<SeanceNumber> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(SeanceNumber), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(SeanceNumber entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(SeanceNumber entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					SeanceNumber SeanceNumber_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(SeanceNumber_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(SeanceNumber));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<SeanceNumber, bool>>(body, param);

					var exsitant_entity = context.SeanceNumbers.Where(lambda_expression).FirstOrDefault();
