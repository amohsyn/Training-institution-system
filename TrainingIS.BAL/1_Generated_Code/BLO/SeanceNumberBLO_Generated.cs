...
{ 
	public partial class BaseSeanceNumberBLO : TrainingIS_BaseBLO<SeanceNumber>{
	    

		public BaseSeanceNumberBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new SeanceNumberDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_SeanceNumber.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_SeanceNumber_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_SeanceNumber_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(SeanceNumber item)
        {
		...
		public virtual IQueryable<SeanceNumber> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<SeanceNumber, bool> Condition = null)
        {
           ...
           IQueryable<SeanceNumber> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(SeanceNumber));
            DataTable entityDataTable = exportService.CreateDataTable(msg_SeanceNumber.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_SeanceNumber_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_SeanceNumber_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_SeanceNumber.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(SeanceNumber), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					SeanceNumber entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new SeanceNumberDAO(_UnitOfWork.context);
			}
			private SeanceNumber Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				SeanceNumber entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new SeanceNumberBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual SeanceNumber Load_if_not_attached_in_current_context(SeanceNumber item)
			{

				SeanceNumber entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.SeanceNumbers.Local.Where(a => a.Id == item.Id);
	...
	public  partial class SeanceNumberBLO : BaseSeanceNumberBLO{
		public SeanceNumberBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
