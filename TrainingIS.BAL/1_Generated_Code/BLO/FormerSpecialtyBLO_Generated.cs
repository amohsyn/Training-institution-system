...
{ 
	public partial class BaseFormerSpecialtyBLO : BaseBLO<FormerSpecialty>{
		...
		public override List<FormerSpecialty> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(FormerSpecialty), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(FormerSpecialty entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(FormerSpecialty entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					FormerSpecialty FormerSpecialty_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(FormerSpecialty_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(FormerSpecialty));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<FormerSpecialty, bool>>(body, param);

					var exsitant_entity = context.FormerSpecialties.Where(lambda_expression).FirstOrDefault();
