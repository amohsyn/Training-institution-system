...
{ 
	public partial class BaseWarningTraineeBLO : TrainingIS_BaseBLO<WarningTrainee>{
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_WarningTrainee_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(WarningTrainee item)
        {
		...
		public virtual IQueryable<WarningTrainee> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<WarningTrainee, bool> Condition = null)
        {
           ...
           IQueryable<WarningTrainee> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(WarningTrainee));
            DataTable entityDataTable = exportService.CreateDataTable(msg_WarningTrainee.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_WarningTrainee_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_WarningTrainee_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_WarningTrainee.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(WarningTrainee), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					WarningTrainee entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new WarningTraineeDAO(_UnitOfWork.context);
			}
			private WarningTrainee Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				WarningTrainee entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new WarningTraineeBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual WarningTrainee Load_if_not_attached_in_current_context(WarningTrainee item)
			{

				WarningTrainee entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.WarningTrainees.Local.Where(a => a.Id == item.Id);
	...
	public  partial class WarningTraineeBLO : BaseWarningTraineeBLO{
		public WarningTraineeBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
