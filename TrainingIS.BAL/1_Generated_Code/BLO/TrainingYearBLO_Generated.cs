...
{ 
	public partial class BaseTrainingYearBLO : TrainingIS_BaseBLO<TrainingYear>{
		...
		public BaseTrainingYearBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new TrainingYearDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_TrainingYear.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_TrainingYear_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_TrainingYear_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_TrainingYear_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public virtual IQueryable<TrainingYear> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<TrainingYear, bool> Condition = null)
        {
           ...
           IQueryable<TrainingYear> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(TrainingYear));
            DataTable entityDataTable = exportService.CreateDataTable(msg_TrainingYear.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_TrainingYear_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_TrainingYear_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_TrainingYear.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(TrainingYear), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					TrainingYear entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new TrainingYearDAO(_UnitOfWork.context);
			}
			private TrainingYear Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
			...
			/// <returns></returns>
			protected virtual TrainingYear Load_if_not_attached_in_current_context(TrainingYear item)
			{

				TrainingYear entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.TrainingYears.Local.Where(a => a.Id == item.Id);
	...
	public  partial class TrainingYearBLO : BaseTrainingYearBLO{
		public TrainingYearBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
