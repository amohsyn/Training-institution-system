...
{ 
	public partial class BaseSchoollevelBLO : TrainingIS_BaseBLO<Schoollevel>{
	    

		public BaseSchoollevelBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new SchoollevelDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_Schoollevel.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_Schoollevel_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Schoollevel_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Schoollevel_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(Schoollevel item)
        {
		...
		public virtual IQueryable<Schoollevel> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Schoollevel, bool> Condition = null)
        {
           ...
           IQueryable<Schoollevel> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Schoollevel));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Schoollevel.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Schoollevel_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Schoollevel_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Schoollevel.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Schoollevel), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Schoollevel entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new SchoollevelDAO(_UnitOfWork.context);
			}
			private Schoollevel Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
	...
	public  partial class SchoollevelBLO : BaseSchoollevelBLO{
		public SchoollevelBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
