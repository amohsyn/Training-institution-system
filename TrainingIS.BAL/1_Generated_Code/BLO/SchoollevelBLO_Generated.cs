...
{ 
	public partial class BaseSchoollevelBLO : BaseBLO<Schoollevel>{
		...
		public override List<Schoollevel> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Schoollevel), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(Schoollevel entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(Schoollevel entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					Schoollevel Schoollevel_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(Schoollevel_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(Schoollevel));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<Schoollevel, bool>>(body, param);

					var exsitant_entity = context.Schoollevels.Where(lambda_expression).FirstOrDefault();
