...
{ 
	public partial class BaseClassroomBLO : TrainingIS_BaseBLO<Classroom>{
	    

		public BaseClassroomBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new ClassroomDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_Classroom.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Classroom_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(Classroom item)
        {
		...
		public virtual IQueryable<Classroom> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Classroom, bool> Condition = null)
        {
           ...
           IQueryable<Classroom> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Classroom));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Classroom.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Classroom_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Classroom_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Classroom.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Classroom), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Classroom entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new ClassroomDAO(_UnitOfWork.context);
			}
			private Classroom Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Classroom entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new ClassroomBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Classroom Load_if_not_attached_in_current_context(Classroom item)
			{

				Classroom entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Classrooms.Local.Where(a => a.Id == item.Id);
	...
	public  partial class ClassroomBLO : BaseClassroomBLO{
		public ClassroomBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
