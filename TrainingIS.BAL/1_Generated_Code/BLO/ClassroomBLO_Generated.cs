...
{ 
	public partial class BaseClassroomBLO : BaseBLO<Classroom>{
		...
		public override List<Classroom> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Classroom), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(Classroom entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(Classroom entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					Classroom Classroom_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(Classroom_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(Classroom));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<Classroom, bool>>(body, param);

					var exsitant_entity = context.Classrooms.Where(lambda_expression).FirstOrDefault();
