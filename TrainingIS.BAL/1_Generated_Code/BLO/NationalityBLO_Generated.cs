...
{ 
	public partial class BaseNationalityBLO : BaseBLO<Nationality>{
		...
		public override List<Nationality> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Nationality), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(Nationality entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(Nationality entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					Nationality Nationality_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(Nationality_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(Nationality));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<Nationality, bool>>(body, param);

					var exsitant_entity = context.Nationalities.Where(lambda_expression).FirstOrDefault();
