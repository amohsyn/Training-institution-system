...
{ 
	public partial class BaseNationalityBLO : TrainingIS_BaseBLO<Nationality>{
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Nationality_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Nationality_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Nationality_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public virtual IQueryable<Nationality> Find_as_Queryable(
			...
			Func<Nationality, bool> Condition = null)
        {
           ...
           IQueryable<Nationality> Query = this.entityDAO
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Nationality_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Nationality_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Nationality.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Nationality), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Nationality entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new NationalityDAO(_UnitOfWork.context);
			}
			private Nationality Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Nationality entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new NationalityBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Nationality Load_if_not_attached_in_current_context(Nationality item)
			{

				Nationality entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Nationalities.Local.Where(a => a.Id == item.Id);
	...
	public  partial class NationalityBLO : BaseNationalityBLO{
		public NationalityBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
