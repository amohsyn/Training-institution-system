...
{ 
	public partial class BaseModuleTrainingBLO : TrainingIS_BaseBLO<ModuleTraining>{
	    

		public BaseModuleTrainingBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new ModuleTrainingDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_ModuleTraining.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Index_ModuleTraining_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(ModuleTraining item)
        {
		...
		public virtual IQueryable<ModuleTraining> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<ModuleTraining, bool> Condition = null)
        {
           ...
           IQueryable<ModuleTraining> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(ModuleTraining));
            DataTable entityDataTable = exportService.CreateDataTable(msg_ModuleTraining.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_ModuleTraining_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_ModuleTraining_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_ModuleTraining.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(ModuleTraining), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					ModuleTraining entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new ModuleTrainingDAO(_UnitOfWork.context);
			}
			private ModuleTraining Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				ModuleTraining entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new ModuleTrainingBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual ModuleTraining Load_if_not_attached_in_current_context(ModuleTraining item)
			{

				ModuleTraining entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.ModuleTrainings.Local.Where(a => a.Id == item.Id);
	...
	public  partial class ModuleTrainingBLO : BaseModuleTrainingBLO{
		public ModuleTrainingBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
