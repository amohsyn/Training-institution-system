...
{ 
	public partial class BaseModuleTrainingBLO : BaseBLO<ModuleTraining>{
		...
		public override List<ModuleTraining> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(ModuleTraining), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(ModuleTraining entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(ModuleTraining entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					ModuleTraining ModuleTraining_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(ModuleTraining_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(ModuleTraining));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<ModuleTraining, bool>>(body, param);

					var exsitant_entity = context.ModuleTrainings.Where(lambda_expression).FirstOrDefault();
