...
{ 
	public partial class BaseAbsenceBLO : BaseBLO<Absence>{
		...
		public BaseAbsenceBLO(UnitOfWork<TrainingISModel> UnitOfWork) : base(new AbsenceDAO(UnitOfWork.context))
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Absence), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(Absence entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(Absence entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					Absence Absence_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(Absence_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(Absence));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<Absence, bool>>(body, param);

					var exsitant_entity = context.Absences.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class AbsenceBLO : BaseAbsenceBLO{
		public AbsenceBLO(UnitOfWork<TrainingISModel> UnitOfWork) : base(UnitOfWork) {}
