...
{ 
	public partial class BaseStateOfAbseceBLO : BaseBLO<StateOfAbsece>{
		...
		public BaseStateOfAbseceBLO(UnitOfWork<TrainingISModel> UnitOfWork) : base(new StateOfAbseceDAO(UnitOfWork.context))
        {
		...
		public override List<StateOfAbsece> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(StateOfAbsece entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(StateOfAbsece entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					StateOfAbsece StateOfAbsece_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(StateOfAbsece_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(StateOfAbsece));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<StateOfAbsece, bool>>(body, param);

					var exsitant_entity = context.StateOfAbseces.Where(lambda_expression).FirstOrDefault();
	...
	public  partial class StateOfAbseceBLO : BaseStateOfAbseceBLO{
		public StateOfAbseceBLO(UnitOfWork<TrainingISModel> UnitOfWork) : base(UnitOfWork) {}
