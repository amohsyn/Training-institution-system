...
{ 
	public partial class BaseActionControllerAppBLO : TrainingIS_BaseBLO<ActionControllerApp>{
		...
		public BaseActionControllerAppBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new ActionControllerAppDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_ActionControllerApp.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_ActionControllerApp_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(ActionControllerApp item)
        {
		...
		public virtual IQueryable<ActionControllerApp> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<ActionControllerApp, bool> Condition = null)
        {
           ...
           IQueryable<ActionControllerApp> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(ActionControllerApp));
            DataTable entityDataTable = exportService.CreateDataTable(msg_ActionControllerApp.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_ActionControllerApp_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_ActionControllerApp_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            ExportService exportService = new ExportService(typeof(ActionControllerApp), typeof(Default_ActionControllerApp_Export_Model));
            DataTable dataTable = exportService.CreateDataTable(msg_ActionControllerApp.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(ActionControllerApp), this.GAppContext);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new ActionControllerAppDAO(_UnitOfWork.context);
			}
			private ActionControllerApp Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				...
				{
					entity = new ActionControllerAppBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual ActionControllerApp Load_if_not_attached_in_current_context(ActionControllerApp item)
			{

				ActionControllerApp entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.ActionControllerApps.Local.Where(a => a.Id == item.Id);
	...
	public  partial class ActionControllerAppBLO : BaseActionControllerAppBLO{
		public ActionControllerAppBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
