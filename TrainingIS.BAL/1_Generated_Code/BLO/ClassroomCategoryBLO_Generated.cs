...
{ 
	public partial class BaseClassroomCategoryBLO : TrainingIS_BaseBLO<ClassroomCategory>{
	    

		public BaseClassroomCategoryBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new ClassroomCategoryDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_ClassroomCategory.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_ClassroomCategory_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_ClassroomCategory_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_ClassroomCategory_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(ClassroomCategory item)
        {
		...
		public virtual IQueryable<ClassroomCategory> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<ClassroomCategory, bool> Condition = null)
        {
           ...
           IQueryable<ClassroomCategory> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(ClassroomCategory));
            DataTable entityDataTable = exportService.CreateDataTable(msg_ClassroomCategory.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_ClassroomCategory_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_ClassroomCategory_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_ClassroomCategory.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(ClassroomCategory), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					ClassroomCategory entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new ClassroomCategoryDAO(_UnitOfWork.context);
			}
			private ClassroomCategory Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
			...
			/// <returns></returns>
			protected virtual ClassroomCategory Load_if_not_attached_in_current_context(ClassroomCategory item)
			{

				ClassroomCategory entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.ClassroomCategories.Local.Where(a => a.Id == item.Id);
	...
	public  partial class ClassroomCategoryBLO : BaseClassroomCategoryBLO{
		public ClassroomCategoryBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
