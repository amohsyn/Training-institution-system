...
{ 
	public partial class BaseMeetingBLO : TrainingIS_BaseBLO<Meeting>{
	    

		public BaseMeetingBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new MeetingDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_Meeting.PluralName;
        }
		...
		public virtual IQueryable<Meeting> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Meeting, bool> Condition = null)
        {
           ...
           IQueryable<Meeting> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Meeting));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Meeting.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Meeting_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Meeting_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Meeting.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Meeting), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Meeting entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new MeetingDAO(_UnitOfWork.context);
			}
			private Meeting Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Meeting entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new MeetingBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Meeting Load_if_not_attached_in_current_context(Meeting item)
			{

				Meeting entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Meetings.Local.Where(a => a.Id == item.Id);
