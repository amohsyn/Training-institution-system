...
{ 
	public partial class BaseTrainingLevelBLO : TrainingIS_BaseBLO<TrainingLevel>{
	    

		public BaseTrainingLevelBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new TrainingLevelDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_TrainingLevel.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_TrainingLevel_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_TrainingLevel_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_TrainingLevel_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(TrainingLevel item)
        {
        ...
        {
            ExportService exportService = new ExportService(typeof(TrainingLevel));
            DataTable entityDataTable = exportService.CreateDataTable(msg_TrainingLevel.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_TrainingLevel_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_TrainingLevel_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_TrainingLevel.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(TrainingLevel), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					TrainingLevel entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new TrainingLevelDAO(_UnitOfWork.context);
			}
			private TrainingLevel Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				TrainingLevel entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new TrainingLevelBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual TrainingLevel Load_if_not_attached_in_current_context(TrainingLevel item)
			{

				TrainingLevel entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.TrainingLevels.Local.Where(a => a.Id == item.Id);
	...
	public  partial class TrainingLevelBLO : BaseTrainingLevelBLO{
		public TrainingLevelBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
