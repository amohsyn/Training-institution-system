...
{ 
	public partial class BaseTrainingLevelBLO : BaseBLO<TrainingLevel>{
		...
		public override List<TrainingLevel> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(TrainingLevel), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(TrainingLevel entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(TrainingLevel entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					TrainingLevel TrainingLevel_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(TrainingLevel_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(TrainingLevel));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<TrainingLevel, bool>>(body, param);

					var exsitant_entity = context.TrainingLevels.Where(lambda_expression).FirstOrDefault();
