...
{ 
	public partial class BaseDisciplineCategoryBLO : TrainingIS_BaseBLO<DisciplineCategory>{
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_DisciplineCategory_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(DisciplineCategory item)
        {
		...
		public virtual IQueryable<DisciplineCategory> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<DisciplineCategory, bool> Condition = null)
        {
           ...
           IQueryable<DisciplineCategory> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(DisciplineCategory));
            DataTable entityDataTable = exportService.CreateDataTable(msg_DisciplineCategory.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_DisciplineCategory_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_DisciplineCategory_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_DisciplineCategory.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(DisciplineCategory), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					DisciplineCategory entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new DisciplineCategoryDAO(_UnitOfWork.context);
			}
			private DisciplineCategory Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				DisciplineCategory entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new DisciplineCategoryBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual DisciplineCategory Load_if_not_attached_in_current_context(DisciplineCategory item)
			{

				DisciplineCategory entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.DisciplineCategories.Local.Where(a => a.Id == item.Id);
	...
	public  partial class DisciplineCategoryBLO : BaseDisciplineCategoryBLO{
		public DisciplineCategoryBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
