...
{ 
	public partial class BaseTaskProjectBLO : TrainingIS_BaseBLO<TaskProject>{
		...
		public BaseTaskProjectBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new TaskProjectDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_TaskProject.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_TaskProject_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_TaskProject_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_TaskProject_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public virtual IQueryable<TaskProject> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<TaskProject, bool> Condition = null)
        {
           ...
           IQueryable<TaskProject> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(TaskProject));
            DataTable entityDataTable = exportService.CreateDataTable(msg_TaskProject.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_TaskProject_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_TaskProject_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            ExportService exportService = new ExportService(typeof(TaskProject), typeof(Default_TaskProject_Export_Model));
            DataTable dataTable = exportService.CreateDataTable(msg_TaskProject.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(TaskProject), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					TaskProject entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new TaskProjectDAO(_UnitOfWork.context);
			}
			private TaskProject Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				TaskProject entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new TaskProjectBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual TaskProject Load_if_not_attached_in_current_context(TaskProject item)
			{

				TaskProject entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.TaskProjects.Local.Where(a => a.Id == item.Id);
	...
	public  partial class TaskProjectBLO : BaseTaskProjectBLO{
		public TaskProjectBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
