...
{ 
	public partial class BaseSectorBLO : TrainingIS_BaseBLO<Sector>{
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Sector_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(Sector item)
        {
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Sector_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Sector_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Sector.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
			...
			/// <returns></returns>
			protected virtual Sector Load_if_not_attached_in_current_context(Sector item)
			{

				Sector entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Sectors.Local.Where(a => a.Id == item.Id);
