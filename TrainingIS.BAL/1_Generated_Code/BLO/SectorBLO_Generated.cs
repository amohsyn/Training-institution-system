...
{ 
	public partial class BaseSectorBLO : BaseBLO<Sector>{
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Sector), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(Sector entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(Sector entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					Sector Sector_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(Sector_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(Sector));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<Sector, bool>>(body, param);

					var exsitant_entity = context.Sectors.Where(lambda_expression).FirstOrDefault();
