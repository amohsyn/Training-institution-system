...
{ 
	public partial class BaseCalendarDayBLO : TrainingIS_BaseBLO<CalendarDay>{
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_CalendarDay_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_CalendarDay_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_CalendarDay_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(CalendarDay item)
        {
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_CalendarDay_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_CalendarDay_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_CalendarDay.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
			...
			/// <returns></returns>
			protected virtual CalendarDay Load_if_not_attached_in_current_context(CalendarDay item)
			{

				CalendarDay entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.CalendarDays.Local.Where(a => a.Id == item.Id);
