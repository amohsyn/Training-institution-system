...
{ 
	public partial class BaseEntityPropertyShortcutBLO : TrainingIS_BaseBLO<EntityPropertyShortcut>{
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_EntityPropertyShortcut_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_EntityPropertyShortcut_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_EntityPropertyShortcut_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(EntityPropertyShortcut item)
        {
		...
		public virtual IQueryable<EntityPropertyShortcut> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<EntityPropertyShortcut, bool> Condition = null)
        {
           ...
           IQueryable<EntityPropertyShortcut> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(EntityPropertyShortcut));
            DataTable entityDataTable = exportService.CreateDataTable(msg_EntityPropertyShortcut.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_EntityPropertyShortcut_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_EntityPropertyShortcut_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_EntityPropertyShortcut.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(EntityPropertyShortcut), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					EntityPropertyShortcut entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new EntityPropertyShortcutDAO(_UnitOfWork.context);
			}
			private EntityPropertyShortcut Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				EntityPropertyShortcut entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new EntityPropertyShortcutBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual EntityPropertyShortcut Load_if_not_attached_in_current_context(EntityPropertyShortcut item)
			{

				EntityPropertyShortcut entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.EntityPropertyShortcuts.Local.Where(a => a.Id == item.Id);
