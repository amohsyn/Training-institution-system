...
{ 
	public partial class BaseLogWorkBLO : TrainingIS_BaseBLO<LogWork>{
		...
		public BaseLogWorkBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new LogWorkDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_LogWork.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_LogWork_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_LogWork_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_LogWork_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(LogWork item)
        {
		...
		public virtual IQueryable<LogWork> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<LogWork, bool> Condition = null)
        {
           ...
           IQueryable<LogWork> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(LogWork));
            DataTable entityDataTable = exportService.CreateDataTable(msg_LogWork.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_LogWork_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_LogWork_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_LogWork.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(LogWork), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					LogWork entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new LogWorkDAO(_UnitOfWork.context);
			}
			private LogWork Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				LogWork entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new LogWorkBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual LogWork Load_if_not_attached_in_current_context(LogWork item)
			{

				LogWork entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.LogWorks.Local.Where(a => a.Id == item.Id);
	...
	public  partial class LogWorkBLO : BaseLogWorkBLO{
		public LogWorkBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
