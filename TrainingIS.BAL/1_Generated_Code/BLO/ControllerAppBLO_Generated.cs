...
{ 
	public partial class BaseControllerAppBLO : TrainingIS_BaseBLO<ControllerApp>{
	    

		public BaseControllerAppBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new ControllerAppDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_ControllerApp.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_ControllerApp_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_ControllerApp_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_ControllerApp_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(ControllerApp item)
        {
		...
		public virtual IQueryable<ControllerApp> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<ControllerApp, bool> Condition = null)
        {
           ...
           IQueryable<ControllerApp> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(ControllerApp));
            DataTable entityDataTable = exportService.CreateDataTable(msg_ControllerApp.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_ControllerApp_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_ControllerApp_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_ControllerApp.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(ControllerApp), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					ControllerApp entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new ControllerAppDAO(_UnitOfWork.context);
			}
			private ControllerApp Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				ControllerApp entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new ControllerAppBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual ControllerApp Load_if_not_attached_in_current_context(ControllerApp item)
			{

				ControllerApp entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.ControllerApps.Local.Where(a => a.Id == item.Id);
	...
	public  partial class ControllerAppBLO : BaseControllerAppBLO{
		public ControllerAppBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
