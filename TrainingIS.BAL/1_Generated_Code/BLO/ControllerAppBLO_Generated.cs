...
{ 
	public partial class BaseControllerAppBLO : BaseBLO<ControllerApp>{
		...
		public override List<ControllerApp> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(ControllerApp), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(ControllerApp entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(ControllerApp entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					ControllerApp ControllerApp_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(ControllerApp_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(ControllerApp));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<ControllerApp, bool>>(body, param);

					var exsitant_entity = context.ControllerApps.Where(lambda_expression).FirstOrDefault();
