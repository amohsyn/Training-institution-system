...
{ 
	public partial class BaseProjectBLO : TrainingIS_BaseBLO<Project>{
	    

		public BaseProjectBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new ProjectDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_Project.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_Project_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Project_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Project_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public virtual IQueryable<Project> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Project, bool> Condition = null)
        {
           ...
           IQueryable<Project> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Project));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Project.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Project_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Project_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Project.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Project), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Project entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new ProjectDAO(_UnitOfWork.context);
			}
			private Project Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Project entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new ProjectBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Project Load_if_not_attached_in_current_context(Project item)
			{

				Project entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Projects.Local.Where(a => a.Id == item.Id);
	...
	public  partial class ProjectBLO : BaseProjectBLO{
		public ProjectBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
