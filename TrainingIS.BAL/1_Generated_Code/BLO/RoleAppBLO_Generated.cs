...
{ 
	public partial class BaseRoleAppBLO : TrainingIS_BaseBLO<RoleApp>{
		...
		public BaseRoleAppBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new RoleAppDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_RoleApp.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_RoleApp_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(RoleApp item)
        {
		...
		public virtual IQueryable<RoleApp> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<RoleApp, bool> Condition = null)
        {
           ...
           IQueryable<RoleApp> Query = this.entityDAO
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_RoleApp_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_RoleApp_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_RoleApp.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(RoleApp), this.GAppContext);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new RoleAppDAO(_UnitOfWork.context);
			}
			private RoleApp Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				...
				{
					entity = new RoleAppBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual RoleApp Load_if_not_attached_in_current_context(RoleApp item)
			{

				RoleApp entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.RoleApps.Local.Where(a => a.Id == item.Id);
	...
	public  partial class RoleAppBLO : BaseRoleAppBLO{
		public RoleAppBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
