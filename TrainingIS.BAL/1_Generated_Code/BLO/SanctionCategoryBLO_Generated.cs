...
{ 
	public partial class BaseSanctionCategoryBLO : TrainingIS_BaseBLO<SanctionCategory>{
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_SanctionCategory_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_SanctionCategory_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_SanctionCategory_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(SanctionCategory item)
        {
		...
		public virtual IQueryable<SanctionCategory> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<SanctionCategory, bool> Condition = null)
        {
           ...
           IQueryable<SanctionCategory> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(SanctionCategory));
            DataTable entityDataTable = exportService.CreateDataTable(msg_SanctionCategory.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_SanctionCategory_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_SanctionCategory_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_SanctionCategory.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(SanctionCategory), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					SanctionCategory entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new SanctionCategoryDAO(_UnitOfWork.context);
			}
			private SanctionCategory Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				SanctionCategory entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new SanctionCategoryBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual SanctionCategory Load_if_not_attached_in_current_context(SanctionCategory item)
			{

				SanctionCategory entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.SanctionCategories.Local.Where(a => a.Id == item.Id);
