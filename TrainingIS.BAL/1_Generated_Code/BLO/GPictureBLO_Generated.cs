...
{ 
	public partial class BaseGPictureBLO : TrainingIS_BaseBLO<GPicture>{
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_GPicture_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_GPicture_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_GPicture_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_GPicture_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_GPicture_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_GPicture.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
			...
			/// <returns></returns>
			protected virtual GPicture Load_if_not_attached_in_current_context(GPicture item)
			{

				GPicture entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.GPictures.Local.Where(a => a.Id == item.Id);
