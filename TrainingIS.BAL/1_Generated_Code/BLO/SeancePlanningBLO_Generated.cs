...
{ 
	public partial class BaseSeancePlanningBLO : TrainingIS_BaseBLO<SeancePlanning>{
		...
		public BaseSeancePlanningBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new SeancePlanningDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_SeancePlanning.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_SeancePlanning_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_SeancePlanning_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(SeancePlanning item)
        {
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_SeancePlanning_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_SeancePlanning_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_SeancePlanning.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
			...
			/// <returns></returns>
			protected virtual SeancePlanning Load_if_not_attached_in_current_context(SeancePlanning item)
			{

				SeancePlanning entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.SeancePlannings.Local.Where(a => a.Id == item.Id);
