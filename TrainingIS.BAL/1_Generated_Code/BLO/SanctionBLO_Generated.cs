...
{ 
	public partial class BaseSanctionBLO : TrainingIS_BaseBLO<Sanction>{
		...
		public BaseSanctionBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new SanctionDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_Sanction.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_Details_Sanction_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Details_Sanction_Model).GetProperties(typeof(SearchByAttribute)))
            {
		...
		public virtual IQueryable<Sanction> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Sanction, bool> Condition = null)
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Sanction), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Sanction entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new SanctionDAO(_UnitOfWork.context);
			}
			private Sanction Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Sanction entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new SanctionBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Sanction Load_if_not_attached_in_current_context(Sanction item)
			{

				Sanction entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Sanctions.Local.Where(a => a.Id == item.Id);
