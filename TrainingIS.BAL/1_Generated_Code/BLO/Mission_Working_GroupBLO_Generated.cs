...
{ 
	public partial class BaseMission_Working_GroupBLO : TrainingIS_BaseBLO<Mission_Working_Group>{
	    

		public BaseMission_Working_GroupBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new Mission_Working_GroupDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_Mission_Working_Group.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Mission_Working_Group_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Mission_Working_Group_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(Mission_Working_Group item)
        {
		...
		public virtual IQueryable<Mission_Working_Group> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Mission_Working_Group, bool> Condition = null)
        {
           ...
           IQueryable<Mission_Working_Group> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Mission_Working_Group));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Mission_Working_Group.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Mission_Working_Group_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Mission_Working_Group_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Mission_Working_Group.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Mission_Working_Group), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Mission_Working_Group entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new Mission_Working_GroupDAO(_UnitOfWork.context);
			}
			private Mission_Working_Group Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Mission_Working_Group entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new Mission_Working_GroupBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Mission_Working_Group Load_if_not_attached_in_current_context(Mission_Working_Group item)
			{

				Mission_Working_Group entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Mission_Working_Groups.Local.Where(a => a.Id == item.Id);
	...
	public  partial class Mission_Working_GroupBLO : BaseMission_Working_GroupBLO{
		public Mission_Working_GroupBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
