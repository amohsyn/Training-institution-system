...
{ 
	public partial class BaseTrainingTypeBLO : TrainingIS_BaseBLO<TrainingType>{
	    

		public BaseTrainingTypeBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new TrainingTypeDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_TrainingType.PluralName;
        }

		public virtual List<string> GetSearchCreteria()
        {
            List<string> SearchCreteria = new List<string>();
            foreach (PropertyInfo model_property in typeof(Default_TrainingType_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_TrainingType_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_TrainingType_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(TrainingType item)
        {
		...
		public virtual IQueryable<TrainingType> Find_as_Queryable(
			...
			Func<TrainingType, bool> Condition = null)
        {
        ...
        {
            ExportService exportService = new ExportService(typeof(TrainingType));
            DataTable entityDataTable = exportService.CreateDataTable(msg_TrainingType.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_TrainingType_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_TrainingType_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_TrainingType.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(TrainingType), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					TrainingType entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new TrainingTypeDAO(_UnitOfWork.context);
			}
			private TrainingType Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				TrainingType entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new TrainingTypeBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual TrainingType Load_if_not_attached_in_current_context(TrainingType item)
			{

				TrainingType entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.TrainingTypes.Local.Where(a => a.Id == item.Id);
	...
	public  partial class TrainingTypeBLO : BaseTrainingTypeBLO{
		public TrainingTypeBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
