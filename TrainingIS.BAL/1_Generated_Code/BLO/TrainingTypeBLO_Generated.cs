...
{ 
	public partial class BaseTrainingTypeBLO : BaseBLO<TrainingType>{
		...
		public override List<TrainingType> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(TrainingType entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(TrainingType entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					TrainingType TrainingType_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(TrainingType_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(TrainingType));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<TrainingType, bool>>(body, param);

					var exsitant_entity = context.TrainingTypes.Where(lambda_expression).FirstOrDefault();
