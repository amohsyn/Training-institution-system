...
{ 
	public partial class BaseMetierBLO : BaseBLO<Metier>{
		...
		public override List<Metier> FindAll()
        {
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Metier), navigationPropertiesNames, foreignKeys);
			...
			/// <returns></returns> 
			public IList<ValidationResult> Validate(Metier entity)
			{
			...
			/// <returns></returns>
			public bool IsUnique(Metier entity, PropertyInfo propertyInfo)
			{
				// Edit Case
				if (entity.Id != 0)
				{
					// Edit Case
					Metier Metier_db = this.FindBaseEntityByID(entity.Id);
					if (propertyInfo.GetValue(Metier_db).ToString() == propertyInfo.GetValue(entity).ToString())
						return true;
				}
				...
				if (propertyInfo.GetValue(entity) != null)
				{
					...
					var param = Expression.Parameter(typeof(Metier));
					var body = Expression.Equal(Expression.Property(param, propertyInfo),
						Expression.Constant(propertyInfo.GetValue(entity)));
					var lambda_expression = Expression.Lambda<Func<Metier, bool>>(body, param);

					var exsitant_entity = context.Metiers.Where(lambda_expression).FirstOrDefault();
