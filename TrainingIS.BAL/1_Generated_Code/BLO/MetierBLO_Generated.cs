...
{ 
	public partial class BaseMetierBLO : TrainingIS_BaseBLO<Metier>{
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Metier_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Metier_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Metier_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(Metier item)
        {
		...
		public virtual IQueryable<Metier> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Metier, bool> Condition = null)
        {
           ...
           IQueryable<Metier> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Metier));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Metier.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Metier_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Metier_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Metier.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Metier), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Metier entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new MetierDAO(_UnitOfWork.context);
			}
			private Metier Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Metier entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new MetierBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Metier Load_if_not_attached_in_current_context(Metier item)
			{

				Metier entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Metiers.Local.Where(a => a.Id == item.Id);
	...
	public  partial class MetierBLO : BaseMetierBLO{
		public MetierBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
