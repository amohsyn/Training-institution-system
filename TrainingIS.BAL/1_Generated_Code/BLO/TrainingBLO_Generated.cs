...
{ 
	public partial class BaseTrainingBLO : TrainingIS_BaseBLO<Training>{
	    

		public BaseTrainingBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new TrainingDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_Training.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_Training_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public override int Save(Training item)
        {
		...
		public virtual IQueryable<Training> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Training, bool> Condition = null)
        {
           ...
           IQueryable<Training> Query = this.entityDAO
        ...
        public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = exportService.CreateDataTable(msg_Training.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Training_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Training_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Training.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Training), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Training entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private Training Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Training entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new TrainingBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Training Load_if_not_attached_in_current_context(Training item)
			{

				Training entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Trainings.Local.Where(a => a.Id == item.Id);
	...
	public  partial class TrainingBLO : BaseTrainingBLO{
		public TrainingBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
