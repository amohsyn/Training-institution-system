...
{ 
	public partial class BaseSeanceTrainingBLO : TrainingIS_BaseBLO<SeanceTraining>{
		...
		public BaseSeanceTrainingBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new SeanceTrainingDAO(UnitOfWork.context),GAppContext)
        {
			...
			this.PluralName = msg_SeanceTraining.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Index_SeanceTraining_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public virtual IQueryable<SeanceTraining> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<SeanceTraining, bool> Condition = null)
        {
           ...
           IQueryable<SeanceTraining> Query = this.entityDAO
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
			...
			/// <returns></returns>
			protected virtual SeanceTraining Load_if_not_attached_in_current_context(SeanceTraining item)
			{

				SeanceTraining entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.SeanceTrainings.Local.Where(a => a.Id == item.Id);
