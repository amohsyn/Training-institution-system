...
{ 
	public partial class BaseJustificationAbsenceBLO : TrainingIS_BaseBLO<JustificationAbsence>{
	    

		public BaseJustificationAbsenceBLO(UnitOfWork<TrainingISModel> UnitOfWork,GAppContext GAppContext) : base(new JustificationAbsenceDAO(UnitOfWork.context),GAppContext)
        {
		    this._UnitOfWork = UnitOfWork;
			this.PluralName = msg_JustificationAbsence.PluralName;
        }
		...
		public virtual List<string> GetSearchCreteria()
        {
			...
			// SearchBy of Entity
            var entity_attributes = typeof(Default_JustificationAbsence_Index_Model).GetCustomAttributes(typeof(SearchByAttribute));
		...
		public virtual IQueryable<JustificationAbsence> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<JustificationAbsence, bool> Condition = null)
        {
           ...
           IQueryable<JustificationAbsence> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(JustificationAbsence));
            DataTable entityDataTable = exportService.CreateDataTable(msg_JustificationAbsence.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_JustificationAbsence_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_JustificationAbsence_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_JustificationAbsence.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(JustificationAbsence), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					JustificationAbsence entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new JustificationAbsenceDAO(_UnitOfWork.context);
			}
			private JustificationAbsence Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				JustificationAbsence entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new JustificationAbsenceBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual JustificationAbsence Load_if_not_attached_in_current_context(JustificationAbsence item)
			{

				JustificationAbsence entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.JustificationAbsences.Local.Where(a => a.Id == item.Id);
	...
	public  partial class JustificationAbsenceBLO : BaseJustificationAbsenceBLO{
		public JustificationAbsenceBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
