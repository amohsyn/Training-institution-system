...
{ 
	public partial class BaseFunctionBLO : TrainingIS_BaseBLO<Function>{
		...
		public virtual List<string> GetSearchCreteria()
        {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Function_Index_Model).GetProperties(typeof(GAppDataTableAttribute)))
            {
            ...
            foreach (PropertyInfo model_property in typeof(Default_Function_Index_Model).GetProperties(typeof(SearchByAttribute)))
            {
		...
		public override int Save(Function item)
        {
		...
		public virtual IQueryable<Function> Find_as_Queryable(
            FilterRequestParams filterRequestParams,
            List<string> SearchCreteria,
            out int totalRecords,
			Func<Function, bool> Condition = null)
        {
           ...
           IQueryable<Function> Query = this.entityDAO
        ...
        {
            ExportService exportService = new ExportService(typeof(Function));
            DataTable entityDataTable = exportService.CreateDataTable(msg_Function.PluralName);
        ...
        public DataTable Export(string Controller_Reference)
        {
            ...
            List<Default_Function_Export_Model> _Exported_Data = null;
            ...
            try
            {
                ...
                _Exported_Data = new Default_Function_Export_ModelBLM(this._UnitOfWork, this.GAppContext)
            ...
            DataTable dataTable = exportService.CreateDataTable(msg_Function.PluralName);
		...
		public virtual ImportReport Import(DataTable dataTable, string FileName = "")
		{
				...
				ImportService importService = new ImportService(dataTable, typeof(Function), this.GAppContext);
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					Operation operation;
					Function entity = this.Load_Or_CreateEntity(importService, entity_reference);
			...
			private void InitUnitOfWork()
			{
				...
				this.entityDAO = new FunctionDAO(_UnitOfWork.context);
			}
			private Function Load_Or_CreateEntity(ImportService importService, string entity_reference)
			{
				Operation operation;
				Function entity = this.FindBaseEntityByReference(entity_reference);
				if (entity == null) // Add new if the entity not exist
				{
					entity = new FunctionBLO(this._UnitOfWork, this.GAppContext).CreateInstance();
			...
			/// <returns></returns>
			protected virtual Function Load_if_not_attached_in_current_context(Function item)
			{

				Function entity = null;

				// if the item is in current context
				var item_in_context = this._UnitOfWork.context.Functions.Local.Where(a => a.Id == item.Id);
	...
	public  partial class FunctionBLO : BaseFunctionBLO{
		public FunctionBLO(UnitOfWork<TrainingISModel> UnitOfWork, GAppContext GAppContext) : base(UnitOfWork,GAppContext) {}
