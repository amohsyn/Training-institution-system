...
namespace  TrainingIS.BLL
{
	public partial class BaseApplicationParamBLO : BaseBLO<ApplicationParam>{
		...
		public BaseApplicationParamBLO(UnitOfWork UnitOfWork) : base()
        {
		    this._UnitOfWork = UnitOfWork;
            this.entityDAO = this._UnitOfWork.ApplicationParamDAO;
        }
		 
		private BaseApplicationParamBLO() : base() {}
		...
		public virtual DataTable Export()
        {
            ...
            DataTable entityDataTable = new DataTable(msg_ApplicationParam.PluralName);

            var foreignKeys = this._UnitOfWork.context.GetForeignKeysIds(typeof(ApplicationParam));
            var Keys =  this._UnitOfWork.context.GetKeyNames(typeof(ApplicationParam));

            var navigationPropertiesNames = this.NavigationPropertiesNames();

            // Create DataColumn Names
            var Properties = typeof(ApplicationParam).GetProperties();
	...
	public virtual string Import(DataTable dataTable)
        {
            ImportService importService = new ImportService(typeof(ApplicationParam), this._UnitOfWork.context);
            ...
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                #region Create or Louad ApplicationParam Instance
                ...
                ApplicationParam entity = this.FindBaseEntityByReference(reference);
                if (entity == null){
                    entity = new ApplicationParam();
            ...
            // Resume
            string resume_msg = string.Format(msgBLO.In_total_there_is_the_insertion_of, number_of_saved) + " " + msg_ApplicationParam.PluralName;
            importService.Report.AddMessage(resume_msg, MessageTypes.Resume_Info);
            resume_msg = string.Format(msgBLO.In_total_there_is_the_update_of, number_of_updated) + " " + msg_ApplicationParam.PluralName;
            importService.Report.AddMessage(resume_msg, MessageTypes.Resume_Info);
    ...
    public virtual string Import_Old(DataTable dataTable)
        {
            ...
            msg += "<hr>";
            msg += string.Format(msgBLO.In_total_there_is_the_insertion_of, number_of_saved) + " " + msg_ApplicationParam.PluralName;
			msg += "<br>";
            msg += string.Format(msgBLO.In_total_there_is_the_update_of, number_of_updated) + " " + msg_ApplicationParam.PluralName;
            return msg;
        }
	...
	public  partial class ApplicationParamBLO : BaseApplicationParamBLO{
		public ApplicationParamBLO(UnitOfWork UnitOfWork) : base(UnitOfWork) {}
